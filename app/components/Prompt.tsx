...
const Prompt = ({
	...
	const handleProfile = () => {
		if (post.userId === session.userId) return router.push('/profile');
	...
	return (
		<div className='break-inside-avoid rounded-lg border border-gray-300 bg-white/20
		 bg-clip-padding backdrop-blur-lg backdrop-filter h-fit w-full px-4 py-4  max-w-sm  min-w-md overflow-x-auto'>
			...
			<div className={`${line ? ' bg-slate-700 py-4 rounded-lg' : ''}`}
				onClick={() => setLine(!line)}>
				<code className={`font-mono text-sm text-gray-700  whitespace-pre-line 
				${line ? 'text-green-600 bg-slate-700 mx-4 ' : 'line-clamp-6 '}`}>
					{post.content}
				</code>
			</div>

			{session?.userId === post.userId && pathname === '/profile' && (
